...
// Constructor
Scene::Scene(int depth) : m_depth(depth), m_topColor(1.0f), m_bottomColor(0.5f) {}

// Overloaded constructor
Scene::Scene(int depth, const color3_t& topColor, const color3_t& bottomColor)
    : m_depth(depth), m_topColor(topColor), m_bottomColor(bottomColor) {}
    ...
    for (int y = 0; y < canvas.GetSize().y; y++)
    {
        for (int x = 0; x < canvas.GetSize().x; x++)
        {
            ...
            for (int s = 0; s < numSamples; ++s)
            {
                ...
                accumulatedColor += Trace(ray, 0, 100, raycastHit, m_depth);
            }
